...
export  const checkKeysetRefExists = async (keysetRefGuardName: string, chainId: ChainId) => {
  ...
  try {
    const transaction = buildKeysetRefGuardTransaction({ chainId, keysetRefGuardName });
    ...
    console.error(`Failed to send transaction to check the keyset defined as "${keysetRefGuardName}" on chain ${chainId}`);
    console.error(error);
    return false;
  }
};
